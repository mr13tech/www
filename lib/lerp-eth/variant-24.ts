...
/** Tangent web — connect opposite-t points on adjacent edges */
const variant24: DrawFn = (ctx, faces, _top, _bot, numLines) => {
  for (let step = 0; step < numLines; step++) {
    ...
    for (const tri of faces) {
      for (let idx = 0; idx < 3; idx++) {
        ...
        const p2 = lerp(tri[(idx + 1) % 3], tri[(idx + 2) % 3], 1 - t)
