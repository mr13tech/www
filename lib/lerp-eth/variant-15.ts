...
const variant15: DrawFn = (ctx, faces, _top, _bot, numLines) => {
  for (let step = 0; step < numLines; step++) {
    ...
    for (const tri of faces) {
      drawLine(ctx, lerp(tri[0], tri[1], t), lerp(tri[0], tri[2], t), '#b3d574', '#24b391', 0.15 * alpha, 0.8)
      drawLine(ctx, lerp(tri[1], tri[0], t), lerp(tri[1], tri[2], t), '#24b391', '#b3d574', 0.15 * alpha, 0.8)
    }
  }
}
